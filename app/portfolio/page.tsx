    ...
    <div className="min-h-screen text-white" style={{ background: '#0a0c0f' }}>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <motion.section id="hero" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="pt-20 pb-24 md:pt-28 md:pb-32">
          <div className="max-w-3xl">
            ...
            <div className="font-mono text-slate-500 space-y-1.5 mb-8 text-sm">
            ...
            <p className="text-slate-500 max-w-xl mb-10 text-sm leading-relaxed">
            ...
            <div className="flex flex-wrap gap-6 mb-10">
              {[
                ...
                <div key={stat.label} className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-xs text-slate-400">{stat.value}</span>
                  <span className="font-mono text-[10px] text-slate-700">{stat.label}</span>
                </div>
              ))}
            </div>
            <div className="flex flex-wrap gap-3">
              <a href="#plugins" className="group flex items-center gap-2 px-4 py-2 font-mono text-[11px] text-slate-400 hover:text-slate-200 transition-colors" style={{ background: 'linear-gradient(180deg, rgba(25,50,75,0.35) 0%, rgba(20,40,65,0.25) 100%)', border: '1px solid rgba(50,90,140,0.18)' }}>
                View Projects <ChevronRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
              </a>
              <a href="#contact" className="flex items-center gap-2 px-4 py-2 font-mono text-[11px] text-slate-600 hover:text-slate-400 transition-colors" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <section id="plugins" className="pb-24">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {projects.map((project, i) => (
              <motion.div key={project.name} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }} className="group relative" style={{ background: 'linear-gradient(180deg, rgba(20,23,30,0.8) 0%, rgba(16,19,25,0.9) 100%)', border: '1px solid rgba(255,255,255,0.04)' }}>
                <div className="px-4 py-2 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5" style={{ background: '#5ba06a' }} />
                    <span className="font-mono text-xs text-slate-300">{project.name}</span>
                  </div>
                  <span className="font-mono text-[9px] text-slate-700">{project.lang}</span>
                </div>
                <div className="p-4">
                  <p className="text-slate-500 text-xs leading-relaxed mb-4">{project.description}</p>
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {project.tags.map((tag) => <span key={tag} className="font-mono text-[9px] px-1.5 py-0.5 text-slate-600" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>{tag}</span>)}
                  </div>
                  <div className="flex items-center font-mono text-[10px] text-slate-600">
                    <a href={project.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 ml-auto text-slate-700 group-hover:text-slate-500 transition-colors cursor-pointer">Source <ArrowUpRight className="w-2.5 h-2.5" /></a>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <section id="servers" className="pb-24">
          ...
          <div className="space-y-3">
            {serverProjects.map((project, i) => (
              <motion.div key={project.name} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.4 }} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 px-4 py-3" style={{ background: 'rgba(16,19,25,0.6)', border: '1px solid rgba(255,255,255,0.03)' }}>
                <div className="shrink-0 sm:w-40">
                  <div className="font-mono text-xs text-slate-300">{project.name}</div>
                  <div className="font-mono text-[10px] text-[#d0a040]">{project.role}</div>
                </div>
                <div className="flex-1 text-xs text-slate-600 leading-relaxed">{project.description}</div>
                <div className="shrink-0 font-mono text-[10px] text-slate-700 flex items-center gap-1"><Users className="w-3 h-3" /> {project.players}</div>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section id="about" className="pb-24">
          <motion.div initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="p-5" style={{ background: 'rgba(16,19,25,0.5)', border: '1px solid rgba(255,255,255,0.03)' }}>
            <div className="flex items-center gap-2 mb-4"><Terminal className="w-4 h-4 text-[#5ba06a]" /><span className="font-mono text-xs text-slate-400">~/about</span></div>
            <div className="font-mono text-[11px] text-slate-600 space-y-2 leading-relaxed">
              <p><span className="text-slate-500">$</span> Systems Engineering student at UNITEC, Honduras. Currently a software developer intern at Red Abierta. Previously backend developer at Marketing Total building authentication systems. Freelancer on Fiverr developing Minecraft plugins and managing server deployments.</p>
              <p><span className="text-slate-500">$</span> I focus on system optimization, security, and scalability. From Java plugins to backend APIs, I build things that are reliable, performant, and well-documented.</p>
              <p><span className="text-slate-500">$</span> Certified in Cisco CCNA: Networking, with additional training in innovation (ASU) and advanced mathematics (UC San Diego).</p>
              <p className="text-[#5ba06a]/60">{'// Code should be read by humans, not just compilers.'}</p>
            </div>
          </motion.div>
        </section>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.03)' }}>
        <div className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
          <span className="font-mono text-[10px] text-slate-800">AlexVila04 — 2026</span>
          <span className="font-mono text-[10px] text-slate-800">Crafted with ☕ & Java</span>
        </div>
      </footer>
    </div>
  );
